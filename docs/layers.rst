...
Most functionality is available with different levels of granularity and abstraction, or requires lower level facilities
for implementation. API level separation is described here. For documentation organized by functionality,
see :doc:`components`.
...
The Python API implementation (bindings to a thin wrapper of the C++ implementation) is used to build a user-friendly
and idiomatic Python interface (module). It's functionality is not necessarily mirrored in the CLI tools or a C++
convenience API.
